...
const Chatbot = () => {
  ...
  // Simple stable ID generator for messages
  const idSeq = useRef(0);
  const newId = () => `${Date.now()}_${++idSeq.current}`;

  // Initialize welcome message after translations are ready
  useEffect(() => {
    setMessages([{ id: newId(), text: t('chatbot.welcome'), sender: 'bot' }]);
  ...
  const sendMessage = async () => {
    ...
    const userMessage = { id: newId(), text: input, sender: 'user' };
